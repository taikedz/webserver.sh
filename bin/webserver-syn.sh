...
##bash-libs: safe.sh @ addb4c5b (2.0.5)
...
util:pid_for() {
    . <(args:use:local procname procport -- "$@") ; 
...
conn:respond() {
    . <(args:use:local input output target -- "$@") ; 
...
http:respond() {
    . <(args:use:local output code message target -- "$@") ; 
...
runmain "webserver.sh" webserver:main "$@"
